...
def get_telebot_user(db: Session, chat_id: int):
    return db.query(models.Telebot_User).filter(models.Telebot_User.chat_id == chat_id).first()

def update_telebot_user(db: Session, chat_id: int, first_name: str, shift: int):
    res = db.query(models.Telebot_User).filter(models.Telebot_User.chat_id == chat_id).first()
    if res:
        res.first_name = first_name
        res.shift = shift
    else: res = models.Telebot_User(chat_id=chat_id, first_name=first_name, shift=shift)
    db.add(res)
    db.commit()
